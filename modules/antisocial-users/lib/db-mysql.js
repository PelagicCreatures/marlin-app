	...
	*/
	newInstance(collectionName, data, cb) {
		...
		this.queryDB(sql, vals, function (err, result) {
			if (err) {
				let e = new VError(err, 'newInstance error');
				return cb(e);
			}
			...
			self.queryDB(sql, [data.id], function (err, fullrow) {
				if (err) {
					let e = new VError(err, 'newInstance read fullrow error');
					return cb(e);
				}
				if (!fullrow || fullrow.length !== 1) {
					let e = new VError(err, 'newInstance read fullrow not found');
					return cb(e);
				}
	...
	/*
		get item from mysql storage

		expects pairs to be an object of
			{
				columnName: value to find,
				columnName: value to find,
				columnName:{ 'compare': '=','<','>' etc, 'value': match }
			}
		pairs are anded

	*/
	getInstances(collectionName, pairs, order, cb) {
		var clauses = [];
		var normalizedPairs = {};

		for (var col in pairs) {
			var compare = '=';
			if (typeof (pairs[col]) === 'string') {
				normalizedPairs[col] = pairs[col]
			}
			else {
				compare = pairs[col].compare;
				normalizedPairs[col] = pairs[col].value
			}
			clauses.push(col + ' ' + compare + ' ?');
		}
		...
		var vals = this.encodeTypes(collectionName, normalizedPairs);
	...
	*/
	updateInstance(collectionName, id, patch, cb) {
		...
		this.queryDB(sql, vals, function (err, result) {
			...
			self.queryDB(sql, [id], function (err, fullrow) {
				...
				self.decodeTypes(collectionName, fullrow);
	...
	deleteInstance(collectionName, id, cb) {
		...
		this.queryDB(sql, [id], function (err, results) {
			...
			self.decodeTypes(collectionName, results);
