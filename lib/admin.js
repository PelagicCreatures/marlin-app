...
function mount(app, db, options) {
	...
	router.get('/:table/create', csrfProtection, userForRequestMiddleware, ensureRoleMiddleware, function (req, res) {
		let table = req.params.table;
	...
	router.get('/:table/:rowId/edit', csrfProtection, userForRequestMiddleware, ensureRoleMiddleware, function (req, res) {
		let table = req.params.table;
		let id = parseInt(req.params.rowId);
	...
	router.post('/:table', express.json({
		limit: '20mb'
	}), csrfProtection, userForRequestMiddleware, ensureRoleMiddleware, function (req, res) {
		let table = req.params.table;
...
class adminMarkdownColumn extends adminColumn {
	...
	getForm(instance, data, options) {
		return pug.renderFile(expressApp.get('views') + '/admin/type-markdown.pug', {
			name: this.table.name + '[' + this.name + ']',
			value: instance ? instance[this.name] : '',
			options: this.options
		});
