	...
	router.post('/:table', express.json({
		...
		app.db.newInstance(table, req.body[table], function (err, instance) {
			...
			admin.handleUpdate(instance, req.body[table], (err, dirty) => {
				...
				app.db.updateInstance(table, instance.id, instance, function (err, instance) {
					if (err) {
						return res.send({
							status: 'error',
							flashLevel: 'danger',
...
class adminTable extends EventEmitter {
	...
	handleUpdate(instance, input, done) {
		this.dirty = false;
		async.each(this.columns, (col, cb) => {
			col.handleUpdate(instance, input, cb)
		}, (err) => {
			done(err, this.dirty);
		})
	}

	delete(instance, input, done) {
		async.each(this.columns, (col, cb) => {
			col.delete(instance, input, cb)
		}, (err) => {
			done(err);
		})
	}
...
class adminColumn extends EventEmitter {
	...
	handleUpdate(instance, body, done) {
...
class adminUploadableColumn extends adminColumn {
	...
	handleUpdate(instance, input, done) {
		...
		if (!pair[1] || !pair[0].match(/^data:/)) {
			return done(new VError('incoming data is not in in expected format'))
		}
		...
		async.series([(cb) => {
			...
			instance[this.name] = JSON.stringify(metadata);
...
class adminImageColumn extends adminUploadableColumn {
	...
	getValue(instance, options) {
		...
		return '<img src="' + UPLOAD_URI_PREFIX + '/' + this.metadata.filename + '" width="' + this.metadata.width + '" height="' + this.metadata.height + '">';
	}
		...
		async.series([
			(cb) => {
				...
				this.table.app.db.getInstances(this.options.references.model, {
					...
					this.currentValue = related[0][adminTables[this.options.references.model].options.defaultColumn];
					cb();
				});
			}, (cb) => {
