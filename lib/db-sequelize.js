			...
			for (let m in this.sequelize.models) {
				...
				for (let assoc in model.associations) {
					...
					if (relationship.throughModel) {
						...
						// let UI know who owns through table records
						if (_.has(model, 'options.ADMIN.isParentOf')) {
							if (model.options.ADMIN.isParentOf.indexOf(relationship.throughModel.tableName) !== -1) {
								relationship.throughModel.options.ADMIN = {
									parent: {
